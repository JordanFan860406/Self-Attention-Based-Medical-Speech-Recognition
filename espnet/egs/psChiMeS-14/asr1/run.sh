...
#resume=exp/train_pytorch_train_pytorch_transformer/results/snapshot.ep.400     # Resume the training from snapshot
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/chimes-14_data_prep.sh ${data}/chimes_14/wav ${data}/chimes_14/transcript
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    text2tokenChimes.py -s 1 -n 1 data/${train_set}/text | cut -f 2- -d" " | tr " " "\n" \
    | sort | uniq | grep -v -e '^\s*$' | awk '{print $0 " " NR+1}' >> ${dict}
    wc -l ${dict}

    echo "make json files"
    data2jsonChimes.sh --feat ${feat_tr_dir}/feats.scp \
		 data/${train_set} ${dict} > ${feat_tr_dir}/data.json
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
        data2jsonChimes.sh --feat ${feat_recog_dir}/feats.scp \
